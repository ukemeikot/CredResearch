            ...
            >
              ...
              {q.isLoading ? (
                ...
                <ul className="mt-6 space-y-3">
                  {items.map((e) => (
                    <li key={e.id} className="rounded-xl border border-white/5 bg-white/[0.02] px-4 py-3">
                      <div className="flex items-center justify-between gap-2">
                        <span className="text-sm text-white">{e.featureKey.replace(/-/g, " ")}</span>
                        <span className="rounded-full border border-white/10 px-2 py-0.5 text-[10px] uppercase tracking-wider text-slate-400">
                          {e.action}
                        </span>
                      </div>
                      {e.suggestionSummary && (
                        <p className="mt-1.5 line-clamp-3 text-xs text-slate-400">{e.suggestionSummary}</p>
                      )}
