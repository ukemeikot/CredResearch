...
export default function AdminPage() {
  ...
  if (!isAdmin) {
    return (
      <div className="mx-auto max-w-2xl py-20 text-center">
        ...
        <p className="mt-2 text-sm text-slate-400">This area is for platform administrators only.</p>
      </div>
    );
  }
  ...
  return (
    <div className="mx-auto max-w-5xl">
      ...
      <p className="mt-1 text-sm text-slate-400">Headline statistics across the platform.</p>
      <div className="mt-6 grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4">
        {cards.map((c) => (
          <GlassCard key={c.label} className="p-5">
            <p className="text-3xl font-bold text-white">{stats.isLoading ? "…" : (c.value ?? 0)}</p>
            <p className="mt-1 text-xs uppercase tracking-wider text-slate-400">{c.label}</p>
          </GlassCard>
        ))}
      </div>
    </div>
