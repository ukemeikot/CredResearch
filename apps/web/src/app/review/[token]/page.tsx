...
export default function ReviewAccessPage({ params }: { params: Promise<{ token: string }> }) {
  ...
  if (loading) {
    return (
      <div className="grid min-h-screen place-items-center bg-cosmos-950">
  ...
  if (error && !view) {
    return (
      <div className="grid min-h-screen place-items-center bg-cosmos-950 px-6 text-center">
        <div>
          <p className="text-lg font-semibold text-white">Review link unavailable</p>
  ...
  return (
    <div className="min-h-screen bg-cosmos-950 px-4 py-10 text-slate-200">
      <div className="mx-auto max-w-2xl">
        ...
        {/* Section content */}
        <div className="mt-6 whitespace-pre-wrap rounded-2xl border border-white/10 bg-white/[0.02] p-5 text-sm leading-relaxed">
        ...
        <div className="mt-6">
          ...
          {view.decisions.map((d) => (
            <p key={d.id} className="mt-1 text-xs text-slate-400">
        ...
        <div className="mt-6">
          ...
          <div className="mt-2 space-y-2">
            ...
            {view.comments.map((c) => (
              <div key={c.id} className="rounded-lg border border-white/10 bg-white/[0.03] px-3 py-2 text-xs">
        ...
        {!decided && (
          <div className="mt-6 rounded-2xl border border-accent/20 bg-accent/[0.03] p-4">
            ...
            <input
              ...
              className="mt-2 w-full rounded-lg border border-white/10 bg-white/[0.05] px-3 py-2 text-sm text-white outline-none"
            />
            <div className="mt-2 flex flex-wrap gap-2">
              {[
                ...
                <button
                  ...
                  onClick={() => decide(b.d)}
                  disabled={busy}
                  className={`inline-flex items-center gap-1 rounded-lg border px-3 py-1.5 text-sm hover:bg-white/5 disabled:opacity-50 ${b.cls}`}
                >
