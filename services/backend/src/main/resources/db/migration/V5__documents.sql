...
-- Idempotent (IF NOT EXISTS + guarded seeds) so an interrupted rolling deploy re-runs cleanly.
...
CREATE INDEX IF NOT EXISTS idx_templates_institution ON templates (institution_id);
...
CREATE TABLE IF NOT EXISTS document_format_rules (
    id               uuid PRIMARY KEY DEFAULT gen_random_uuid(),
    template_id      uuid NOT NULL REFERENCES templates(id),
...
CREATE INDEX IF NOT EXISTS idx_format_rules_template ON document_format_rules (template_id);
...
CREATE INDEX IF NOT EXISTS idx_document_sections_document ON document_sections (document_id, order_index);
CREATE INDEX IF NOT EXISTS idx_document_sections_search_tsv ON document_sections USING gin (search_tsv);
...
CREATE INDEX IF NOT EXISTS idx_document_versions_section ON document_versions (document_section_id, version);

-- ── Seed: three global templates (UG / MSc / PhD), Proposal + Ch1–3 (FR-TMPL-1/3) ──
-- Guarded so a re-run never duplicates the seed.
...
INSERT INTO document_format_rules (template_id, font_family, font_size_pt, line_spacing, heading_numbering, citation_style, margins_json)
SELECT v.tid, 'Times New Roman', 12, 2.0, 'decimal', v.cs,
       '{"top":1.0,"bottom":1.0,"left":1.5,"right":1.0,"unit":"in"}'::jsonb
FROM (VALUES
        ('11111111-1111-1111-1111-111111111111'::uuid, 'APA'),
        ('22222222-2222-2222-2222-222222222222'::uuid, 'APA'),
        ('33333333-3333-3333-3333-333333333333'::uuid, 'IEEE')
     ) AS v(tid, cs)
WHERE NOT EXISTS (SELECT 1 FROM document_format_rules d WHERE d.template_id = v.tid);
...
WHERE NOT EXISTS (SELECT 1 FROM template_sections ts WHERE ts.template_id = t.id);
